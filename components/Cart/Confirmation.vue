<template>
    <div class="max-w-3xl flex flex-col gap-9 mx-auto">
        <div class="flex items-center justify-center flex-col">
            ...
            <div class="h-20 w-20 bg-third flex items-center justify-center rounded-full mb-5">
        ...
        <div class="border border-head p-10">
            ...
            <div class="flex flex-col divide-y divide-footer">
                <div class="py-4 flex items-center justify-between text-sm font-medium uppercase text-head">
                    <span>PRODUCT</span>
                    <span>SUBTOTAL</span>
                </div>
                <div class="py-4 flex flex-col gap-4">
                    <div v-for="item in orderDetails.orderitems" :key="item.id"
                ...
                <div class="py-4 flex items-center justify-between text-sm font-medium text-head">
                    <span class="uppercase">SHIPPING</span>
                    <span class="font-normal">Free shipping</span>
                </div>

                <div class="py-4 hidden items-center justify-between text-sm font-medium uppercase text-head">
                    <span>VAT</span>
                    <span>$19</span>
                </div>
        ...
        <div class="flex items-center justify-end">
            <button class="text-sm font-medium text-white bg-head px-6 py-4 flex items-center justify-center w-48"
                @click="makePayment" >
                PAYMENT
            </button>
        </div>
    ...
    methods: {
        ...
        async makePayment() {
            try {
                ...
                await this.redirectToPayment(data.payload);
            } catch (error) {
                console.log("payment:", error);
            }
        },

        async redirectToPayment(payload) {
            const options = {
            ...
            await this.$axios.request(options).then(function (response) {
